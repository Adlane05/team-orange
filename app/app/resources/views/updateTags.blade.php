...
<nav id="sidebar" style="width: 200px; background-color: black; color: white; height: 100%; padding-top: 20px; position: relative; float: left;">
  ...
  <main style="height:100%; margin-left: 200px;">
    ...
    <div style="text-align:center; display:flex; width:60%; margin-left:20%; margin-right:20%; margin-top:5vh;">
        ...
        <div>
            <form id="updateTag" action="" method="POST"> 
            @csrf
                <label for="tag" style="font-size:40px">Tag</label> <br>
                <input type="text" name="tag" id="tag" autocomplete="off" value="{{$tag->tag_name}}" style="border-radius:15px; border:3px solid black; height:3vh; width:14vw; padding:10px; font-size:24px;">
                @if($errors->has('tag'))
                    <p style="height:1vh;">{{$errors->first('tag')}}</p>
                @else
                <p style="height:1vh;"></p>
                @endif
                <button type="submit" form="updateTag" style="border-radius:25px; font-size:40px; height:6vh; width:15vw; margin-top: 2vh; background-color:red; color:white; font-weight:bold">
                    Update Tag
                </button>
                <input type="hidden" value="{{$tag->tag_id}}" name="originalTagCode">
            </form>
        </div>
